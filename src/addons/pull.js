...
if (!process.argv.includes("-")) {
    ...
    childProcess.execSync(
        `git clone --depth=1 --branch=tw https://github.com/TinyPatch/addons ${repoPath}`
    );
}
...
const detectUnimplementedAPIs = (addonId, contents) => {
    ...
    if (contents.includes("addon.self.dir")) {
        ...
        console.warn(
            `Warning: ${addonId} contains unwritten addon.self.dir. It or this script should be modified so that it will be rewritten.`
        );
    }
    ...
    if (contents.includes("addon.self.lib")) {
        // eslint-disable-next-line max-len
        console.warn(
            `Warning: ${addonId} contains unwritten addon.self.lib. It should use modern ES6 import statements.`
        );
    }
};
