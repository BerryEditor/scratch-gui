...
export default async function ({ addon, console }) {
    ...
    const primaryColor = (category) => {
        if (addon.self.disabled)
            return originalColors[category.colorId].primary;
    ...
    const secondaryColor = (category) => {
        if (addon.self.disabled)
            return originalColors[category.colorId].secondary;
        if (isColoredTextMode())
            return alphaBlend(
                primaryColor(category),
                multiply(addon.settings.get(category.settingId), { a: 0.15 })
            );
        if (textMode() === "black")
    ...
    const tertiaryColor = (category) => {
        if (addon.self.disabled)
            return originalColors[category.colorId].tertiary;
    ...
    const fieldBackground = (category) => {
        ...
        if (
            category instanceof Blockly.Block ||
            category instanceof Blockly.Field
        ) {
            const block =
                category instanceof Blockly.Block
                    ? category
                    : category.sourceBlock_;
            if (isColoredTextMode() || textMode() === "black") {
                let primary;
                if (block.isShadow() && block.getParent())
                    primary = block.getParent().getColour();
        ...
        if (isColoredTextMode())
            return alphaBlend(
                primaryColor(category),
                multiply(addon.settings.get(category.settingId), { a: 0.25 })
            );
        if (textMode() === "black")
    ...
    Blockly.Toolbox.Category.prototype.createDom = function () {
        ...
        if (this.iconURI_) {
            ...
            if (
                !["sa-blocks", "videoSensing", "text2speech"].includes(this.id_)
            )
            ...
            if (match) {
                ...
                const category =
                    this.id_ === "sa-blocks" ? saCategory : extensionsCategory;
                const newColor = isColoredTextMode
                    ? tertiaryColor(category)
                    : primaryColor(category);
                if (newColor) {
        ...
        if (!category) return;
        this.bubble_.style.backgroundColor = isColoredTextMode()
            ? fieldBackground(category)
            : primaryColor(category);
    ...
    Blockly.FieldDropdown.prototype.init = function () {
        ...
        if (textColor(this) !== "#ffffff")
            this.arrow_.style.filter = recolorFilter(textColor(this));
    };
    ...
    const oldFieldMatrixUpdateMatrix =
        Blockly.FieldMatrix.prototype.updateMatrix_;
    ...
    const oldFieldMatrixCreateButton =
        Blockly.FieldMatrix.prototype.createButton_;
    ...
    const updateColors = () => {
        ...
        for (const category of categories) {
            ...
            Blockly.Colours[category.colorId].secondary =
                secondaryColor(category);
            Blockly.Colours[category.colorId].tertiary =
                tertiaryColor(category);
        }
        ...
        if (uncoloredTextColor() === "#575e75")
            Blockly.Colours.fieldShadow = "rgba(0, 0, 0, 0.15)";
